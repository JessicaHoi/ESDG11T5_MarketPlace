    ...
    <div class="pt-16 flex-1 flex flex-col" v-if="listing">
      ...
      <div class="bg-ink text-paper px-6 py-4 flex items-center justify-between border-b border-white/10">
        ...
        <div class="hidden sm:flex items-center gap-3 bg-white/5 border border-white/10 px-3 py-2">
          <div class="w-8 h-8 overflow-hidden bg-white/10 flex-shrink-0">
            <img :src="listing.image" :alt="listing.title" class="w-full h-full object-cover" />
          </div>
          <div>
            <p class="font-display font-semibold text-xs leading-tight truncate max-w-[160px]">{{ listing.title }}</p>
            <p class="font-mono text-xs text-accent">${{ currentOffer || listing.price }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-1 overflow-hidden">
          ...
          <div ref="messagesContainer" class="flex-1 overflow-y-auto p-6 space-y-4 max-h-[calc(100vh-280px)]">
            ...
            <div class="text-center">
              <span class="section-label bg-cream px-3 py-1.5 text-muted">
                Negotiation started · {{ listing.title }}
              </span>
            </div>
              ...
              <div class="max-w-[70%]">
                ...
                >
                  ...
                  <p class="text-xs text-muted font-mono mt-1">
                    {{ msg.sender === 'buyer' ? `Down from $${listing.price}` : `Original: $${listing.price}` }}
                  </p>
        ...
        <div class="hidden lg:block w-64 border-l border-ink/10 p-5 bg-cream/50">
          ...
          <div class="aspect-square bg-cream overflow-hidden mb-3">
            <img :src="listing.image" :alt="listing.title" class="w-full h-full object-cover" />
          </div>
          <p class="font-display font-semibold text-sm leading-tight mb-2">{{ listing.title }}</p>
          <div class="flex items-baseline gap-2 mb-3">
            <span class="font-display font-bold text-xl text-accent">${{ currentOffer || listing.price }}</span>
            <span v-if="currentOffer && currentOffer !== listing.price" class="font-mono text-xs text-muted line-through">${{ listing.price }}</span>
          </div>
          <div class="space-y-2 text-xs font-mono">
            <div class="flex justify-between">
              <span class="text-muted">Condition</span>
              <span class="text-ink">{{ listing.condition }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">Location</span>
              <span class="text-ink">{{ listing.location }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-muted">Seller rating</span>
              <span class="text-ink">★ {{ listing.sellerRating }}</span>
            </div>
          </div>
...
async function simulateSellerCounterOffer(buyerOffer) {
  ...
  const listed = listing.value.price
...
function proceedToPurchase() {
  if (agreedPrice.value) {
    router.push(`/purchase/${listing.value.id}?price=${agreedPrice.value}`)
  }
}
