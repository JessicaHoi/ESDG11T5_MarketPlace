  ...
  <div class="min-h-screen bg-paper">
    ...
    <div class="pt-16">
      <div class="max-w-4xl mx-auto px-6 pt-8">
        <router-link to="/disputes" class="section-label text-muted hover:text-accent transition-colors">← My Disputes</router-link>
      </div>
      ...
      <div v-else-if="dispute" class="max-w-4xl mx-auto px-6 py-6">
        ...
        <div v-if="dispute.deadlineAt && !isResolved" class="mb-6 border p-4 flex items-center gap-3"
          ...
          <div class="flex-1">
            <p class="font-display font-semibold text-sm" :class="timerExpired ? 'text-red-800' : 'text-amber-800'">{{ timerExpired ? 'Deadline expired' : 'Time remaining' }}</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div class="lg:col-span-2 space-y-5">
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="bg-cream p-4">
                  ...
                  <div class="flex items-center gap-2">
                    <div class="w-8 h-8 bg-accent/20 text-accent font-display font-bold text-xs flex items-center justify-center">{{ mockUser.name.charAt(0) }}</div>
                    <div><p class="font-display font-semibold text-sm">{{ mockUser.name }}</p><p class="text-xs text-muted font-mono">Filed the dispute</p></div>
                  </div>
                </div>
                <div class="bg-cream p-4">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <div><p class="font-display font-semibold text-sm">Seller #{{ dispute.sellerID }}</p><p class="text-xs text-muted font-mono">Responding party</p></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="bg-cream border border-ink/10 p-5">
              ...
              <div v-if="dispute.sellerResponse"><p class="text-ink text-sm leading-relaxed">{{ dispute.sellerResponse }}</p></div>
              <div v-else class="flex items-center gap-2 text-amber-400"><span class="text-sm">⏱</span><p class="text-sm font-mono">Awaiting response</p></div>
            </div>
            ...
            <div class="bg-cream border border-ink/10 p-5">
              ...
              <div class="max-h-64 overflow-y-auto space-y-3 mb-4" ref="threadContainer">
                ...
                <div v-for="msg in threadMessages" :key="msg.messageID || msg.id" class="flex"
                  :class="msg.senderID === mockUser.id ? 'justify-end' : 'justify-start'">
                  <div class="max-w-[80%] px-3 py-2 text-sm"
                    :class="msg.senderID === mockUser.id ? 'bg-accent/10 border border-accent/20' : 'bg-white border border-ink/10'">
                    <p class="font-mono text-xs text-muted mb-1">{{ msg.senderID === mockUser.id ? 'You' : `Seller #${msg.senderID}` }}</p>
                    <p>{{ msg.content }}</p>
              ...
              <div v-if="!isResolved" class="flex gap-2">
                <input v-model="newThreadMsg" type="text" class="input-field flex-1" placeholder="Reply to seller..." @keyup.enter="sendThreadMessage" />
          ...
          <div class="lg:col-span-1">
            <div class="bg-cream border border-ink/10 p-5 sticky top-24">
              ...
              <div class="bg-cream p-3 text-xs font-mono space-y-1.5 mb-4">
                ...
                <div class="flex justify-between"><span class="text-muted">Seller responded</span><span :class="dispute.sellerResponse ? 'text-sage' : 'text-amber-400'">{{ dispute.sellerResponse ? 'Yes' : 'No' }}</span></div>
              ...
              <div v-if="isResolved" class="p-4 text-center"
                ...
                <p class="text-xs text-muted font-mono mt-1">{{ dispute.disputeStatus === 'APPROVED' ? 'You will be refunded' : 'Funds released to seller' }}</p>
              </div>

              <div v-else class="border border-amber-200 bg-amber-50 p-3">
                <p class="text-xs text-amber-700 leading-relaxed">⏱ Your dispute is under review. The seller has until {{ formatDate(dispute.deadlineAt) }} to respond. If no response is received, you'll be automatically refunded.</p>
              </div>
            </div>
          </div>
        </div>
