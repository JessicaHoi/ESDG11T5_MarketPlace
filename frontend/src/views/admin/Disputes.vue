    ...
    <div class="pt-16">
      ...
      <div class="bg-ink text-paper py-10 px-6">
        <div class="max-w-5xl mx-auto">
          ...
          <h1 class="font-display font-extrabold text-4xl">Dispute Management</h1>
        </div>
      </div>
        ...
        <template v-else>
          ...
          <div class="space-y-3">
            <div
              ...
              @click="$router.push(`/admin/disputes/${dispute.disputeID}`)"
            >
              <div class="p-5 flex items-center gap-4">
                <div class="w-1.5 self-stretch flex-shrink-0 rounded-full" :class="statusBar(dispute.disputeStatus)"></div>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-3 mb-2 flex-wrap">
                    <span class="font-mono font-medium text-sm text-ink">DIS-{{ dispute.disputeID }}</span>
                    <span
                      class="inline-flex items-center px-2 py-0.5 text-xs font-mono font-medium uppercase tracking-wider"
                      :class="statusBadge(dispute.disputeStatus)"
                    >{{ dispute.disputeStatus }}</span>
                    <span class="font-mono text-xs text-muted">ORD-{{ dispute.orderID }}</span>
                  </div>
                  ...
                  <div class="flex items-center gap-4 flex-wrap">
                    <span class="text-xs text-slate font-mono">Buyer #{{ dispute.buyerID }}</span>
                    <span class="text-xs text-slate font-mono">Seller #{{ dispute.sellerID }}</span>
                    <span class="text-xs text-muted font-mono">{{ formatDate(dispute.createdAt) }}</span>
                  </div>
                </div>
                <div class="text-right flex-shrink-0">
                  <p class="font-display font-bold text-lg text-accent">${{ dispute.amount || 0 }}</p>
                  <p class="text-xs text-muted font-mono mt-1">{{ (dispute.disputeReason || '').replace(/_/g, ' ') }}</p>
                  <div class="flex items-center gap-1 justify-end mt-2">
                    <span class="text-xs text-muted font-mono">Deadline: {{ formatDate(dispute.deadlineAt) }}</span>
                  </div>
                </div>
...
const stats = computed(() => [
  ...
  { label: 'Awaiting', value: disputes.value.filter(d => d.disputeStatus === 'AWAITING_DECISION').length, color: 'text-blue-400' },
...
const filteredDisputes = computed(() => {
  return disputes.value.filter(d => {
    ...
    const matchSearch = !search.value ||
      String(d.disputeID).includes(search.value) ||
      (d.listingTitle || '').toLowerCase().includes(search.value.toLowerCase()) ||
      String(d.buyerID).includes(search.value)
...
function statusBar(status) {
  return { OPEN: 'bg-amber-400', RESPONSE: 'bg-blue-400', AWAITING_DECISION: 'bg-purple-400', APPROVED: 'bg-sage', REJECTED: 'bg-red-500' }[status] || 'bg-ink/20'
}

function statusBadge(status) {
  return { OPEN: 'bg-amber-100 text-amber-700', RESPONSE: 'bg-blue-100 text-blue-700', AWAITING_DECISION: 'bg-purple-100 text-purple-700', APPROVED: 'bg-sage/20 text-sage', REJECTED: 'bg-red-100 text-red-600' }[status] || 'bg-ink/10 text-slate'
}
